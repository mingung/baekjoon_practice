...
int main() {
    ...
    while(1) {
        Touch = XGpio_DiscreteRead(&GpioTouch, TOUCH_CHANNEL);
				...
				// 2개 터치가 동시에 눌린 경우 2씩 증가
         if((Touch & TOUCH_ONE) && (Touch & TOUCH_TWO)) { // 증가 버튼
            ...
            if(count < MAX_COUNT && Touch == 0) count += 2;
            printf("increase count = %d\n", count);
            DisplayNumber(count);
        }
				 else if((Touch & TOUCH_ONE) && (Touch & TOUCH_THREE)) { // 증가 버튼
					if(count < MAX_COUNT) count += 2;
            printf("increase count = %d\n", count);
            DisplayNumber(count);
				}

				 else if((Touch & TOUCH_ONE) && (Touch & TOUCH_FOUR)) { // 증가 버튼
					if(count < MAX_COUNT) count += 2;
            printf("increase count = %d\n", count);
            DisplayNumber(count);
				}

				 else if((Touch & TOUCH_TWO) && (Touch & TOUCH_THREE)) { // 증가 버튼
					if(count < MAX_COUNT) count += 2;
            printf("increase count = %d\n", count);
            DisplayNumber(count);
				}

				 else if((Touch & TOUCH_TWO) && (Touch & TOUCH_FOUR)) { // 증가 버튼
					if(count < MAX_COUNT) count += 2;
            printf("increase count = %d\n", count);
            DisplayNumber(count);
				}

				 else if((Touch & TOUCH_THREE) && (Touch & TOUCH_FOUR)) {// 증가 버튼
					if(count < MAX_COUNT) count += 2;
            printf("increase count = %d\n", count);
            DisplayNumber(count);
				}
